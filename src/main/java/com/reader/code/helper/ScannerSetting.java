...
public class ScannerSetting {
    ...
    public boolean defaultSettings() throws Exception {
        ...
        CodeReaderHelper.getDefaultHelper().getReader().setScannerAttr(map);

        CodeReaderHelper.getDefaultHelper().getReader().setScannerAttr(Command.CODE_ID_DISABLE.getBytes(),null);
        CodeReaderHelper.getDefaultHelper().getReader().setScannerAttr(Command.AIM_ID_DISABLE.getBytes(),null);
        Thread.sleep(300);
        CodeReaderHelper.getDefaultHelper().getReader().setScannerAttr(Command.TERMINAL_SYMBOL_DISABLE.getBytes(),null);
        return true;
    }
}
